...
Future<InvoicePdfDesignRail> loadInvoicePdfDesignRail({
  ...
  if (catalogPath != null && isCatalogAssetImageRef(catalogPath)) {
    final raster = await _loadBundledCatalogRaster(catalogPath);
...
Future<img.Image?> _loadBundledCatalogRaster(String path) async {
  try {
