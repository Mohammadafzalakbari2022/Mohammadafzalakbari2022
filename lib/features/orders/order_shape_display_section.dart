            ...
            return Column(
              ...
              children: [
                ...
                if (figure.presetName.isNotEmpty ||
                    figure.textOptionLabels.isNotEmpty ||
                    figure.sizeOptionLabels.isNotEmpty) ...[
                  const SizedBox(height: 4),
                  _FigureOptionLines(
                    labels: labels,
                    presetName: figure.presetName,
                    textOptionLabels: figure.textOptionLabels,
                    sizeOptionLabels: figure.sizeOptionLabels,
                  ),
                ],
              ],
            );
...
class _FigureOptionLines extends StatelessWidget {
  const _FigureOptionLines({
    ...
    required this.textOptionLabels,
    required this.sizeOptionLabels,
  });
    ...
    return Column(
      ...
      children: [
        if (presetName.isNotEmpty)
          Text('${labels.preset}: $presetName', style: style),
        if (textOptionLabels.isNotEmpty)
          Text(
            '${labels.text}: ${textOptionLabels.join(', ')}',
            style: style,
          ),
        if (sizeOptionLabels.isNotEmpty)
          Text(
            '${labels.size}: ${sizeOptionLabels.join(', ')}',
            style: style,
          ),
      ],
    );
