...
class OrderComposerMeasurementsPanel extends ConsumerStatefulWidget {
  ...
  ConsumerState<OrderComposerMeasurementsPanel> createState() =>
      OrderComposerMeasurementsPanelState();
}

class OrderComposerMeasurementsPanelState
  ...
  void didUpdateWidget(OrderComposerMeasurementsPanel oldWidget) {
    ...
    final externalReseed =
        oldWidget.initialProfileId != widget.initialProfileId ||
            ...
            !measurementSnapshotItemsEqual(
              oldWidget.initialItems,
              widget.initialItems,
            );
  ...
  /// Reads live field values (call before save to avoid stale draft).
  OrderMeasurementsEditorResult? readCurrentResult() {
    final types =
        ref.read(measurementTypesStreamProvider).valueOrNull ?? const [];
    if (types.isEmpty) return null;
    return _buildResult(types);
  }
  ...
  OrderMeasurementsEditorResult _buildResult(List<MeasurementTypeSummary> types) {
    ...
    for (final entry in _byTypeId.entries) {
      values[entry.key] = entry.value.text;
    }
    return buildOrderMeasurementsEditorResult(
      types: types,
      valuesByTypeId: values,
      unitCode: ref.read(defaultMeasurementUnitProvider),
