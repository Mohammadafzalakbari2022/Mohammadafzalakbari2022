...
class OrderShapeDisplayItem {
  const OrderShapeDisplayItem({
    ...
    this.imageRef = '',
    this.presetName = '',
    this.textOptionLabels = const [],
    this.sizeOptionLabels = const [],
  });
  ...
  final List<String> textOptionLabels;
  final List<String> sizeOptionLabels;
}
...
OrderShapeDisplayItem _fromSnapshotFigure(OrderStyleSnapshotFigureView figure) {
  return OrderShapeDisplayItem(
    ...
    presetName: figure.presetNameSnapshot.trim(),
    textOptionLabels: figure.textOptions
        .map((o) => o.labelSnapshot.trim())
        .where((s) => s.isNotEmpty)
        .toList(growable: false),
    sizeOptionLabels: figure.sizeOptions
        .map(_sizeOptionLabel)
        .where((s) => s.isNotEmpty)
        .toList(growable: false),
  );
}
...
List<OrderShapeDisplayItem> _fromShapeItems(
  ...
  return sortedItems.map((item) {
    ...
    return OrderShapeDisplayItem(
      ...
      shapeName: shapeName,
      imageRef: imageRef,
      presetName: item.presetNameSnapshot?.trim() ?? '',
      textOptionLabels: item.textOptions
          .map((o) => o.labelSnapshot.trim())
          .where((s) => s.isNotEmpty)
          .toList(growable: false),
      sizeOptionLabels: item.sizeOptions
          .map(_sizeOptionLabelFromSnapshot)
          .where((s) => s.isNotEmpty)
          .toList(growable: false),
    );
  }).toList(growable: false);
...
String _sizeOptionLabel(OrderShapeSizeSnapshotView option) {
  final label = option.labelSnapshot.trim();
  if (label.isNotEmpty) return label;
  if (option.valueSnapshot > 0) {
    return '${option.valueSnapshot} ${option.unitSnapshot.trim()}'.trim();
  }
  return '';
}
...
List<String> _buildDetailedLines({
  ...
  for (final figure in figures) {
    ...
    if (figure.presetName.isNotEmpty) {
      add('${labels.preset}: ${figure.presetName}');
    }
    if (figure.textOptionLabels.isNotEmpty) {
      add('${labels.text}: ${figure.textOptionLabels.join(', ')}');
    }
    if (figure.sizeOptionLabels.isNotEmpty) {
      add('${labels.size}: ${figure.sizeOptionLabels.join(', ')}');
    }
  }
...
String _buildCompactPreview({
  ...
  for (final figure in figures) {
    ...
    if (figure.presetName.isNotEmpty) detailParts.add(figure.presetName);
    detailParts.addAll(figure.textOptionLabels);
    detailParts.addAll(figure.sizeOptionLabels);
