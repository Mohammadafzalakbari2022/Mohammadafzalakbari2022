...
OrderSummary _sampleOrder() => OrderSummary(
      internalId: 'ord-1',
      shopId: 'shop-1',
...
Future<void> _pumpTile(
  ...
  required bool enableStatusActions,
  bool detailed = false,
}) async {
...
void main() {
  group('OrderListTile', () {
    testWidgets('shows 5-digit order badge and customer name', (tester) async {
      await _pumpTile(tester, order: _sampleOrder(), enableStatusActions: false);
    ...
    testWidgets('shows status action when enableStatusActions is true',
        (tester) async {
      await _pumpTile(
        tester,
        order: _sampleOrder(),
      ...
      expect(find.byTooltip('Change status'), findsOneWidget);
    });

    testWidgets('hides status action when enableStatusActions is false',
        (tester) async {
      await _pumpTile(
        tester,
        order: _sampleOrder(),
